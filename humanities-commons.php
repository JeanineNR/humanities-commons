...
//		add_action( 'shibboleth_set_user_roles', array( $this, 'hcommons_sync_bp_profile' ) );
		...
		add_filter( 'bp_notifications_get_notifications_for_user', array(
			$this,
			'hcommons_bbp_format_buddypress_notifications'
		) );
	...
	public function hcommons_set_groups_query_args( $args ) {
		...
		if ( bp_is_user_profile() || is_network_admin() || bp_is_current_component( 'settings' ) ) {
			return $args;
		}
	...
	public static function hcommons_get_user_memberships() {
		...
		if ( in_array( $membership_header, $gr_degrees ) ) {
